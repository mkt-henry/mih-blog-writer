...
const COLORS: Record<AgencySlug, string> = {
  mih_speaker: "#1565C0",
  mih_casting: "#7B1FA2",
  mih_agency: "#2E7D32",
  other: "#E65100",
...
export default function AgencyChart({ daily }: Props) {
  if (daily.length === 0) return <div className="text-xs text-gray-400">No data</div>;

  const max = Math.max(10, ...daily.map((d) => AGENCY_SLUGS.reduce((sum, k) => sum + d[k], 0)));
  const W = 800;
  const H = 200;
          ...
          return (
            <g key={d.date}>
              {AGENCY_SLUGS.map((k) => {
                ...
                y -= h;
                return <rect key={k} x={x} y={y} width={w} height={h} fill={COLORS[k]} />;
              })}
            </g>
          );
        })}
