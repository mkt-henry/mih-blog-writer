...
export default function KeywordClient({ keywords, categories }: Props) {
  ...
  const [pubFilter, setPubFilter] = useState<"전체" | "발행" | "미발행">("전체");
  ...
  const filtered = useMemo(() => {
    ...
    return keywords.filter((k) => {
      ...
      if (pubFilter === "발행" && !k.published_url) return false;
      if (pubFilter === "미발행" && k.published_url) return false;
      return true;
    });
  }, [keywords, query, cat, agency, pubFilter]);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-xs text-gray-500 border-b border-gray-100">
              ...
              <th className="text-left px-3 py-2 font-medium w-16">발행</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((k) => (
              <tr key={k.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-3 py-2">
                  {k.published_url ? (
                    <a href={k.published_url} target="_blank" rel="noreferrer" className="text-emerald-600 hover:underline text-xs font-medium">발행</a>
                  ) : <span className="text-gray-300 text-xs">—</span>}
                </td>
              </tr>
            ))}
