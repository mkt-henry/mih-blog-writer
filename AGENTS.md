...
8. **`npm run build`** 를 실행한다 — manifest.js 갱신 + 미업로드 원고 DB 업로드가 한 번에 처리된다.

   ```bash
   npm run build
   ```

   - 마지막 줄에 `완료: 성공 N개 / 실패 0개`가 출력되면 정상이다.
   - 실패가 1개 이상이면 `.env.local`의 `SUPABASE_URL` · `SUPABASE_SERVICE_ROLE_KEY`를 확인한다.
...
- [ ] `npm run build` 실행 — `완료: 성공 N개 / 실패 0개` 확인
