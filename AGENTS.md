...
5. 우측 상단 **콘텐츠 복사** → 명함 포함 HTML이 클립보드에 복사됨
6. **글쓰기 URL 복사** → `https://blog.naver.com/{slug}/postwrite` 가 복사됨. 브라우저 주소창에 붙여넣어 네이버 블로그 글쓰기 페이지를 연다.
7. HTML 모드에 콘텐츠를 붙여넣고 발행한다.

이력 탭(`mih_history`)은 이미 발행된 원고를 모아 보는 영역이라 글쓰기 URL 버튼이 비활성화된다.
