...
### 키워드 밀도
- 메인 키워드(아티스트명+섭외)는 본문 전체에 **10~15회**를 기본 목표로 자연스럽게 분산한다. 긴 원고에서 문맥상 필요할 때만 최대 20회까지 허용한다.
- 억지 반복, 같은 문장 패턴 반복, 소제목 전부에 키워드 삽입은 어뷰징 위험이 있으므로 금지한다.
...
- [ ] 해시태그 단락 20개 이상
- [ ] 메인 키워드(아티스트명+섭외) 본문 10~15회 기본, 최대 20회 이하
