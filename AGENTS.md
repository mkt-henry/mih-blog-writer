...
사용자가 다음처럼 특정 인물이 아닌 **카테고리 키워드**로 요청하면 `지침/04_카테고리_키워드_원고_작성_지침.md`를 우선 적용해 완성된 HTML 원고 파일을 만든다.
...
**카테고리 키워드 출력 파일명 규칙:** `output/[메인키워드]_[원고제목].html`

- `[메인키워드]`에는 `행사공연`, `개그맨섭외`, `래퍼섭외`, `유튜버섭외`처럼 사용자가 요청한 핵심 검색어를 넣는다.
- 카테고리 원고 제목은 `[메인키워드]`로 시작한다.
- 예: `output/개그맨섭외_[개그맨섭외] 분위기를 살리는 MC·토크쇼·기업 행사 섭외.html`
