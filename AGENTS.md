...
모든 결과물은 **네이버 스마트에디터(SE3) HTML 복붙용**으로 출력되며, 네이버 검색 **상위 노출(C-Rank · D.I.A.+ · AI 브리핑/AI탭 인용)** 을 목표로 한다.
...
1. **분기 판단** — 사용자 요청이 인물 1명 섭외인지, 카테고리 키워드 원고인지 결정한다. 헷갈리면 `docs/지침/00_개요.md`를 본다.
   - **인물·키워드를 지정하지 않은 요청**이면 기본은 인물 섭외 원고다. 발행 계정 성격(스피커→강연, 그 외→가수 등 연예인 섭외)에 맞는 새 인물을 고르되, 이미 발행됐거나 발행 대기인 인물/키워드(DB `articles` 전체 + `output/`)는 제외한다.
1-1. **중복 검사 게이트 (작성 착수 전 필수)** — 인물이 정해지면(사용자 지정이든 랜덤 추출이든) 아래를 먼저 돌린다.

   ```bash
   node scripts/check-keyword.mjs "인물명1" "인물명2"
   ```

   `⛔`(종료코드 1)가 하나라도 나오면 **그 인물은 작성하지 않는다.** 다른 후보로 교체하거나 사용자에게 확인받는다.
   `articles`(person_name + 제목 `[인물명]`) · `output/` 대기 원고 · `keywords.published_url` 을 한 번에 대조하고,
   `팬타곤 키노`↔`키노`, `HAON(김하온)`↔`김하온` 같은 **표기 변형**까지 잡는다.
   전체 중복 현황은 `node scripts/diag-keyword-dupe.mjs` 로 감사한다.
...
6. **이미지 업로드 스크립트를 실행한다** — 인물 원고에만 적용. 공식 SNS 또는 소속사/공식 프로필 페이지 이미지 URL을 Supabase 버킷에 업로드하고 HTML src를 교체한다.
   ...
   - 카테고리 원고는 `04_카테고리_키워드_원고_작성_지침.md`에 따라 DB에 이미 저장된 관련 아티스트 이미지를 사용하므로, 새 외부 이미지 업로드 단계는 건너뛴다.
...
2026년 6월 기준 네이버 검색은 기존 C-Rank·D.I.A. 평가를 유지하면서 AI 브리핑, AI탭, 네이버 메이트처럼 **출처 신뢰도·전문 창작자·AI 답변 인용**을 더 강하게 드러내는 방향으로 고도화되고 있다. 따라서 새 원고는 단순 키워드 반복보다 아래 기준을 우선한다.

- **C-Rank 대응:** 발행 계정의 주제 정체성을 흐리지 않는다. `mih_speaker`는 강연·강사·스피커, `mih_casting`/`mih_agency`는 공연·가수·연예인 섭외 문맥을 유지하고, 같은 계정 안에서 무관한 생활 정보나 잡다한 키워드를 섞지 않는다.
- **D.I.A. 대응:** 문서의 주제 적합도, 경험 정보, 정보 충실성, 검색 의도, 독창성, 적시성을 모두 채운다. 프로필 나열만으로 끝내지 말고 "이 인물이 어떤 행사에서 왜 맞는가"를 행사 담당자 관점으로 해석한다.
- **AI 브리핑/AI탭 대응:** 도입부, 섭외 프로세스, FAQ는 AI가 그대로 요약·인용하기 쉬운 짧은 답변형 문장으로 쓴다. 한 문단 안에서 `누구/무엇`, `적합 행사`, `섭외 경로`, `비용 변동 요인`, `신청 절차`가 분명해야 한다.
- **출처 신뢰 대응:** 인물 프로필과 최근 활동은 반드시 웹 검색으로 최신 사실을 확인한다. 단일 출처나 상충 정보는 단정하지 않고, 원고에는 확인된 사실과 일반화 표현만 남긴다.
- **독창성 대응:** 상위 글에서 흔한 표현을 복제하지 않는다. 행사 규모, 관객층, 무대 구성, 섭외 전 확인사항처럼 메이드인헤븐의 실무 판단이 드러나는 문장을 넣는다.
- **어뷰징 방지:** 메인 키워드는 자연스러운 분산이 우선이다. 같은 문장 구조로 반복하거나 모든 소제목에 키워드를 넣지 않는다.
...
- **영문 전용 이름은 한글 독음을 병기한다:** `[DJ PLUMM(플럼) 섭외] …`. 파일명 슬러그와 `person_name`은 원본 영문 그대로 둔다. 상세는 `docs/지침/02_원고_작성_지침.md`의 "인물명 표기" 절.

### 본문 구조 (해시태그 제외 1,500자 이상, 6,000자 초과는 경고)

상위 노출 문서 실측치가 3,883~5,823자다. 이 구간에 들어가지 못하면 같은 쿼리에서 경쟁이 되지 않는다.

**늘리는 방법이 중요하다.** 기존 문장을 길게 풀어 쓰거나 같은 내용을 다시 말하는 것은 금지다(반복은 감점 요인).
아래 "의미 커버리지" 항목을 **각각 다른 어휘로** 채워 분량을 만든다.

1. **도입부(400~600자)** — AEO 정답 노드: 누구이고, 어떤 행사에 최적이며, 섭외 경로는 어디인지를 압축 제시
2. **아티스트 소개(600~800자)** — 프로필 표 포함. 대표곡·활동·무대 스타일을 서술
3. **무대 방향 3가지(500~700자)** — 왼쪽 세로선 박스 ①②③
4. **적합 행사 유형 4종(600~800자)** — 2열 표(행사 유형 / 잘 맞는 이유). 유형별로 **무대 구성·러닝타임·관객 반응 포인트**까지 쓴다
5. **섭외 프로세스 단계별 안내(500~700자)** — 3~4단계 (초기 문의 → 조건·출연료 협의 → 계약 및 공연 준비 → 당일 진행). 단계별 **준비물·리드타임·변수**를 함께 쓴다
6. **행사 규모·예산별 가이드(500~700자)** — 규모별 무대/음향 조건, 비용 변동 요인. 성수기·지역·촬영 여부에 따른 차이
7. **섭외 전 확인 포인트 3가지(400~500자)** — 왼쪽 세로선 박스 ①②③
8. **FAQ 5개(600~800자)** — Q&A 형식. 비용, 적합 행사 유형, 신청 절차, 일정 리드타임, 무대·음향 요건. AI 검색 인용률을 높이는 핵심 섹션
9. **무대 영상** — 유튜브 iframe 임베드 항상 정확히 2개
10. **마무리 CTA** — 카카오톡 오픈채팅 링크 (명함은 모아보기가 자동 합성하므로 본문에 넣지 않는다)
11. **해시태그 단락** — 20개 이상 (`섭외` 포함 태그는 7개 이하)
12. **이미지 삽입 및 출처 표기** — 본문 이미지 `<img>` 4개 + 이미지 출처 표기 4개 고정
...
**절대 횟수가 아니라 밀도로 관리한다.** 네이버는 키워드 출현 횟수가 아니라 쿼리와 문서의 의미 정합성으로 순위를 매긴다. 같은 단어를 반복하면 표현만 단조로워질 뿐 정합성은 오르지 않는다.

- `섭외`는 본문(해시태그 제외) **1000자당 3~5회**. 5회를 넘기면 발행 불가.
- 해시태그 20개 이상은 그대로 유지하되, 그중 **`섭외`가 들어간 태그는 7개 이하**로 제한한다. 태그에 키워드를 몰아넣는 것도 같은 반복 신호다.
- 억지 반복, 같은 문장 패턴 반복, 소제목 전부에 키워드 삽입은 금지한다.
- 위 세 가지는 `npm run check:article`이 하드 검사한다.

### 의미 커버리지 (분량은 여기서 나온다)

`{인물명} 섭외`를 검색하는 행사 담당자의 실제 궁금증을 항목별로, **서로 다른 어휘로** 다룬다.
같은 말을 바꿔 쓰는 게 아니라 각 항목이 다른 정보를 담아야 한다.

| 항목 | 다뤄야 할 내용 |
|---|---|
| 비용 | 출연료 구간, 변동 요인(지역·시간대·촬영 여부), 부대비용 |
| 일정 | 리드타임, 성수기, 일정 조율 절차 |
| 무대 | 러닝타임, 세트 구성, 음향·조명 요건, 관객 규모별 차이 |
| 행사 적합성 | 유형별로 왜 맞는지, 관객층 반응 |
| 준비 | 계약 확인사항, 당일 진행, 담당자가 미리 정할 것 |

**실제 섭외 후기는 쓰지 않는다.** 진행한 행사의 날짜·장소·현장 반응을 지어내는 것은 창작이며,
"사실 확인 원칙"과 "출처 불명 사실 단정 금지"에 정면으로 걸린다.
검증이 필요 없는 섭외 실무 지식과 웹 검색으로 확인되는 아티스트 정보로만 채운다.
...
- 이미지 소스 우선순위는 공식 인스타그램 → 공식 페이스북·X·홈페이지·소속사 프로필 → 기타 이미지다. 본인이 나온 이미지면 일상·화보·비공식 SNS 등도 사용할 수 있다.
- **보도자료 이미지만 저작권 문제로 사용하지 않는다.** 그 외 본인이 나온 이미지는 사용 가능하다.
- 출처는 실제 소스와 무관하게 러프하게 표기한다 (기본 `출처 - [아티스트명] 공식 SNS`).
...
- [ ] 본문 텍스트 1,500자 이상 (해시태그 제외. 6,000자 초과는 경고)
- [ ] 프로필 표 ↔ 본문 중복 없음
- [ ] 모든 문장 끝에 `<br><br>` 적용 (후처리 스크립트 실행 여부)
- [ ] 도입부 300~500자 AEO 정답 노드 포함
- [ ] 도입부 첫 2문단 안에 누구/적합 행사/섭외 경로/문의 방식이 답변형으로 포함됨
- [ ] 최근 활동·소속·프로필 정보가 2026년 기준 웹 검색으로 확인됨
...
- [ ] FAQ 5개 (비용·적합 행사·신청 절차·일정 리드타임·무대 요건) 포함
- [ ] 행사 규모·예산별 가이드 섹션 포함
- [ ] 왼쪽 세로선 박스 2세트 각 3행
...
- [ ] 해시태그 단락 20개 이상
- [ ] 제목에 `섭외` 포함 — **발행 불가 기준** (가장 강한 순위 신호)
- [ ] `섭외` 밀도 15회/1000자 이하 (5회 초과는 경고. 순위와 무관함이 실측으로 확인됨)
- [ ] `섭외` 포함 해시태그 7개 이하
- [ ] 비용·일정·무대·행사 적합성·준비 항목이 각각 다른 어휘로 채워짐
- [ ] 행사 담당자 관점의 독창적 해석(관객층, 무대 구성, 예산·일정 변수)이 포함됨
