...
2. `docs/지침/00_개요.md`로 인물/카테고리 분기를 판단한다.
   ...
   - **인물·키워드 미지정 요청**이면 기본은 인물 섭외 원고. 4계정 모두 일반 섭외 계정이고(2026-09-07 부로 강연 전용 계정 없음 — 스피커 계정은 `influence` 로 개칭, 내부 슬러그는 `mih_speaker` 유지), 이미 발행/발행 대기된 인물·키워드(DB + `output/`)는 제외한다.
   - **`계정별로 N개씩 써줘`처럼 개수만 지정한 요청**은 `05_랜덤_키워드_셀렉트_지침.md`를 따른다. `node scripts/pick-keywords.mjs <agency>=<n>`으로 미작성 후보를 랜덤 추출 → 후보 통보 후 바로 `01→02→03`. 승인 대기는 사용자가 명시적으로 요청했을 때만.
