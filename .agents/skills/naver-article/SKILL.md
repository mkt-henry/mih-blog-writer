...
- 강연·강사·스피커 → `mih_speaker`(표시 이름 `influence`, 블로그 gdfdhzgfgfhgdj)
- 가수·아이돌 → 4계정 중 최근에 덜 쓴 쪽 (2026-09-07 부로 `mih_speaker`=`influence` 도 일반 섭외 계정)
- **인물·키워드 미지정 요청**이면 기본은 인물 섭외 원고. 발행 계정에 귀속된 키워드(`keywords.agency`)에서 골라 **이미 발행됐거나 발행 대기인 인물·키워드(DB `articles` 전체 + `output/`)를 제외**하고 새 인물을 고른다.
