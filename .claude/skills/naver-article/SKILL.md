...
> **응답 압축 스타일 예외**: 이 스킬 실행 중에는 응답 압축·간소화 스타일(caveman, ponytail 등)을 원고 본문과 작업 절차에 적용하지 않는다. 원고는 완전한 문장의 정상 산문으로 쓰고, 지정된 분량 하한을 지킨다. 중복 검사·자료 수집(WebSearch 5회+/WebFetch 1회+)·이미지 4개 확보·기계 검증·publish 등 아래 절차는 어떤 이유로도 생략하거나 축약하지 않는다. 사용자와의 대화 문체만 세션 스타일을 따른다.
