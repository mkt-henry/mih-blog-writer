...
export const DEFAULT_CONFIG = {
  ...
  kakaoUrl: "https://open.kakao.com/o/squEahWg",
...
export function withDerivedUrls(config, request) {
  ...
  const businessCardImageUrl = config.businessCardImageUrl || "";
  const businessCardVersion = config.updatedAt
    ? String(Date.parse(config.updatedAt) || "").replace(/\D/g, "")
    : "";
  const businessCardHtmlUrl = businessCardImageUrl
    ? (businessCardVersion ? `${businessCardImageUrl}?v=${businessCardVersion}` : businessCardImageUrl)
    : "";
  const width = config.businessCardWidth || DEFAULT_CONFIG.businessCardWidth;
  return {
    ...DEFAULT_CONFIG,
    ...config,
    siteBaseUrl: origin,
    kakaoRedirectUrl: `${origin}/go/kakao`,
    businessCardPublicUrl: businessCardImageUrl,
    businessCardHtmlUrl,
    businessCardLinkUrl: BUSINESS_CARD_LINK_URL,
    businessCardHtml: businessCardHtmlUrl
      ? `<p align="center"><a href="${BUSINESS_CARD_LINK_URL}"><img src="${businessCardHtmlUrl}" width="${width}"></a></p>`
      : ""
  };
}
...
export async function readAgencyConfig(slug, request) {
  try {
    const { data } = await supabase
      .from("agencies")
      .select("kakao_url, business_card_image_url, business_card_width, rss_url, site_base_url, updated_at")
      .eq("slug", slug)
      .single();
