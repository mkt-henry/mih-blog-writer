 ...
 *
 * 단순 부분문자열 비교는 쓰지 않는다 — "이브"↔"아이브", "지민"↔"김지민" 처럼
 * 남남인 인물이 대량으로 걸린다(실측 50건 중 대부분이 오탈락이었다).
...
export function excludeReason(name, excludedSet) {
  ...
  for (const ex of excludedSet) {
    if (!ex || ex.length < MIN_PREFIX_LEN) continue; // 빈 문자열/1글자는 접두 비교 제외
    if (kn.length < MIN_PREFIX_LEN) continue;
