...
/**
 * 검색에 실제로 쓸 이름 — **한글 표기를 고른다** (2026-09-02 신설).
 *
 * 우리가 겨루는 것은 한글 검색 순위 하나뿐이다. 검색자는 `DJ PLUMM` 을 철자대로 치지 않고
 * `플럼` 이라고 친다. 그런데 등록명은 `2NE1 (투애니원)` 처럼 영문이 앞에 오거나 괄호가 붙어
 * 있어서, 그대로 검색하면 아무도 치지 않는 문자열을 재게 된다
 * (괄호가 붙은 검색어가 결과 0건을 만든 사고가 실제로 있었다).
 *
 * 규칙: 괄호 밖에 한글이 있으면 그쪽, 없고 괄호 안에 한글이 있으면 그쪽.
 * 양쪽 다 한글이 없으면(`SF9`, `Danny Cho` 등 27건) 원문을 그대로 쓴다 —
 * 한글 표기를 지어내지 않는다.
 *
 *   "아이유"            → "아이유"
 *   "2NE1 (투애니원)"    → "투애니원"
 ...
 *   "SF9"              → "SF9"
 */
export const searchName = (s) => {
  const raw = String(s ?? '').trim();
  const hasKor = (t) => /[가-힣]/.test(t);
  const m = raw.match(/^([^（(]*)[（(]([^）)]*)[）)](.*)$/);
  if (!m) return raw;
  const outside = `${m[1]} ${m[3]}`.replace(/\s+/g, ' ').trim();
  const inside = m[2].trim();
  if (hasKor(outside)) return outside;
  if (hasKor(inside)) return inside;
  return outside || raw;
};
...
export function namesOf(row) {
  // 괄호 안이 한글인 표기("CAMO(카모)")는 norm 이 괄호를 통째로 버려 "camo" 만 남는다.
  // 블로그에는 한글 표기("카모")로 발행되므로 그대로면 같은 인물을 못 알아보고
  // 발행 완료된 원고가 계속 '발행 대기'로 남는다(2026-08-27 카모 사고).
  // searchName 이 고르는 한글 표기를 별칭으로 함께 등록해 양쪽을 잇는다.
  const bracket = String(row?.title ?? '').match(/^\s*\[([^\]]+)\]/);
  const candidates = [
    norm(row?.person_name),
    titleName(row?.title),
    norm(searchName(row?.person_name)),
    bracket ? norm(searchName(bracket[1].replace(ROLE_SUFFIX, '').trim())) : '',
  ];
